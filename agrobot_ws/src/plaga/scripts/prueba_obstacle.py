...
def rrt_star(start, goal, map_array, iterations=3500, delta_q=3.8, goal_tolerance=5, bias_factor=0.3):
...
class MapSubscriber:
    def __init__(self):
        ...
        self.desired_thickness = 3      #Map obstacle thickness
        self.path_key = False            
    ...
    def odom_callback(self, data):
        if self.path_key: 
            ...
            self.goal = ((data.pose.pose.position.x + self.origin_x)/self.map_resolution * -1, (data.pose.pose.position.y + self.origin_y)/self.map_resolution * -1)
