...
def pendulum_simulator():
    rospy.init_node('pendulum_simulator_no_control', anonymous=True)
    rate = rospy.Rate(1)  # Frecuencia de publicacion de 100 Hz

    position_pub = rospy.Publisher('nc_pendulum_position', Vector3, queue_size=10)
    velocity_pub = rospy.Publisher('nc_pendulum_velocity', Vector3, queue_size=10)
